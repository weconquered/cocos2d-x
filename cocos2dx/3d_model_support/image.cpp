...
bool CImage::Load(char * szFilename, bool bMipMap)
{
	...
	{
		CCLOG( "Could not open %s", szFilename);
		return false;
	}
	...
	if(!m_ucpBuffer)
	{
		CCLOG( "Could not alloc memory for %s", szFilename);
		return false;
	}
	...
	CCLOG( "Could not load %s, Invalid filetype", szFilename);
	return false;
}
...
bool CImage::LoadBMP()
{
	...
	{
		delete [] m_ucpBuffer;
		CCLOG( "%s is an invalid BMP file", m_szFilename);
		return false;
	}
	...
	if(m_bMipMap)
	{
		//cjh gluBuild2DMipmaps(GL_TEXTURE_2D, 3, m_uiWidth, m_uiHeight, GL_RGB, GL_UNSIGNED_BYTE, ucpData); 
	}
	else
	{
	...
	CCLOG("BMP Image: %s Loaded", m_szFilename);
...
bool CImage::LoadTGA(bool bCompressed)
{
	...
	{
		CCLOG( "%s is an invalid TGA file", m_szFilename);
		delete [] m_ucpBuffer;
		return false;
	}
	...
	if(m_bMipMap)
	{
		//cjh gluBuild2DMipmaps(GL_TEXTURE_2D, uiType, m_uiWidth, m_uiHeight, uiType, GL_UNSIGNED_BYTE, ucpData); 
	}
	else
	{
	...
	delete [] ucpData;
	delete [] m_ucpBuffer;
	CCLOG("TGA Image: %s Loaded", m_szFilename);
	return true;
}
...
bool CImage::LoadPCX()
{
	...
	{
		CCLOG( "%s is not a valid PCX texture", m_szFilename);
		delete [] m_ucpBuffer;
	}
	ucpPtr += 4;
	...
	if(ucpPtr[0] != 12)
	{
		CCLOG( "Bad palette in %s", m_szFilename);
		delete [] ucpPixData;
		return false;
	}
	ucpPtr++;
	...
	CCLOG("PCX Image: %s Loaded", m_szFilename);
