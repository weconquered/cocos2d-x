...
CCScrollLayerExt::CCScrollLayerExt(CCRect mRect)
{
	...
	contentLayer = CCLayerColor::create(ccc4(128, 128, 128, 0), mRect.size.width, mRect.size.height);
	...
	if (verticalScrollIndicator == NULL)
	{
		...
		verticalScrollIndicator = CCLayerColor::create(ccc4(50, 50, 50, 180), vSize.width,vSize.height);
		verticalScrollIndicator->ignoreAnchorPointForPosition(false);
		...
		verticalScrollIndicator->setTouchEnabled(false);
	...
	if (horizontalScrollIndicator == NULL)
	{
		...
		horizontalScrollIndicator = CCLayerColor::create(ccc4(50, 50, 50, 180), hSize.width,hSize.height);
		horizontalScrollIndicator->ignoreAnchorPointForPosition(false);
		horizontalScrollIndicator->setPosition(ccp(hSize.width/2.0,hSize.height/2.0));
		horizontalScrollIndicator->setTouchEnabled(false);
...
void CCScrollLayerExt::constraintContent()
{
	...
	if (isLimitX ||  isLimitY) {
		...
		CCCallFunc * callBack = CCCallFunc::create(this, callfunc_selector(CCScrollLayerExt::scrollingEnd));
		contentLayer->runAction(CCSequence::createWithTwoActions(fadeOut, callBack));
	}
	else {
		contentLayer->setPosition(ccp( properX,properY));
...
void CCScrollLayerExt::updateIndicators(float mTime)
{
    this->horizontalScrollIndicator->setVisible(this->getShowsHorizontalScrollIndicator());
	this->verticalScrollIndicator->setVisible(this->getShowsVerticalScrollIndicator());
...
{
	if (mAnimate) {
		CCEaseOut * fadeOut = CCEaseOut::create(CCMoveTo::create(0.3,mOffset), 3);
		CCCallFunc * callBack = CCCallFunc::create(this, callfunc_selector(CCScrollLayerExt::constraintContent));
		contentLayer->runAction(CCSequence::createWithTwoActions(fadeOut, callBack));
	}
	else {
...
void CCScrollLayerExt::ccTouchEnded(CCTouch *touch, CCEvent* event)
{
	...
	if (touch == beginTouch) {
		...
		double microSeconds	= CCTime::timersubCocos2d(&beginTime, &endTime);
